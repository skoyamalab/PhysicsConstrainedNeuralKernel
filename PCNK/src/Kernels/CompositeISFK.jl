...
# Our kernel implementation, but with fixed β for more stable training
# Constructors of the kernel require not only a wave number, but also orders for the analytical and neural components as well as β.

function DirectedResidualKernel{T, N}(k::U, β::Real, Ord1::Integer, Ord2::Integer) where {T<:AbstractFloat, N<:Integer, U<:Number}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end

function DirectedResidualKernel{T}(k::U, β::Real, Ord1::N, Ord2::Integer) where {T<:AbstractFloat, N<:Integer, U<:Number}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end

function DirectedResidualKernel(k::T, β::Real, Ord1::N, Ord2::Integer) where {T<:AbstractFloat, N<:Integer}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end

function DirectedResidualKernel{T, N}(k::U, β::AbstractVector{<:Real}, Ord1::Integer, Ord2::Integer) where {T<:AbstractFloat, N<:Integer, U<:Number}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end

function DirectedResidualKernel{T}(k::U, β::AbstractVector{<:Real}, Ord1::N, Ord2::Integer) where {T<:AbstractFloat, N<:Integer, U<:Number}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end

function DirectedResidualKernel(k::T, β::AbstractVector{<:Real}, Ord1::N, Ord2::Integer) where {T<:AbstractFloat, N<:Integer}
    return DirectedResidualKernel{T, N}(MultiDirectionalKernel{T}(k, β, Ord1), NeuralWeightPlaneWaveKernel{T, N}(k, Ord2))
end
