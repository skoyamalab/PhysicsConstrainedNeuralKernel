...
function (a::FixedDirectionSFKernel)(x::AbstractVector{<:Number})
    β = view(a.β, 1)
    return j0.(sqrt(sum((Complex.(a.k*x) - im*(β .*a.v)).^2)))/i0.(β)
end

function (a::FixedDirectionSFKernel)(x::AbstractMatrix)::AbstractVector
    β = view(a.β, 1)
    return j0.(sqrt.(reshape(sum((Complex.(a.k*x) .- im*(β .*a.v)).^2, dims=1), :)))/i0.(β)
end

function (a::FixedDirectionSFKernel)(ΔX::AbstractArray{<:Number, 3})::AbstractMatrix#Format for input is coordinates, vertical batch and horizontal batch
    β = view(a.β, 1)
    d, B1, B2 = size(ΔX)
    return j0.(sqrt.(reshape(sum((a.k*ΔX  - im*repeat(reshape(β .*a.v, (d, 1, 1)), 1, B1, B2)).^2, dims=1), B1, B2)))/i0.(β)
end
...
function (a::FixedDirectionSFKernel)(x1::AbstractVector{<:Number}, x2::AbstractVector{<:Number})
    Δx = Complex.(a.k*(x1 - x2))
    β = view(a.β, 1)
    return j0(sqrt(sum((Δx - im*(β .*a.v)).^2)))/i0.(β)
end

function (a::FixedDirectionSFKernel{<:Number})(X1::AbstractMatrix{<:Number}, x2::AbstractVector{<:Number})::AbstractVector
    Δx = X1 .- x2
    return j0.(sqrt.(reshape(sum((Complex.(a.k*Δx) .- im*(a.β .*a.v)).^2, dims=1), :)))./i0.(a.β)
end
...
function (a::VariableDirectionSFKernel)(x::AbstractVector{<:Number})
    return j0(sqrt(sum((a.k*x - im*a.β).^2)))/i0(norm(a.β))
end

function (a::VariableDirectionSFKernel)(X::AbstractMatrix{<:Number})::AbstractVector
    return j0.(sqrt.(reshape(sum((a.k*X .- im*a.β).^2, dims=1), :)))/i0(norm(a.β))
end

function (a::VariableDirectionSFKernel)(ΔX::AbstractArray{<:Number, 3})::AbstractMatrix
    d, B1, B2 = size(ΔX)
    return j0.(sqrt.(reshape(sum((a.k*ΔX - im*repeat(reshape(a.β, (d, 1, 1)), 1, B1, B2)).^2, dims=1), B1, B2)))/i0(norm(a.β))
end

function (a::VariableDirectionSFKernel)(x1::AbstractVector{<:Number}, x2::AbstractVector{<:Number})
    Δx = Complex.(a.k*(x1 - x2))
    return j0(sqrt(sum((Δx - im*a.β).^2)))/i0(norm(a.β))
end

function (a::VariableDirectionSFKernel)(X1::AbstractMatrix, x2::AbstractVector)::AbstractVector
    Δx = Complex.(a.k*(X1 .- x2))
    return j0.(sqrt.(reshape(sum((Δx .- im*a.β).^2, dims=1), :)))/i0(norm(a.β))
end

function (a::VariableDirectionSFKernel)(x1::AbstractVector{<:Number}, X2::AbstractMatrix{<:Number})::AbstractVector
    Δx = Complex.(a.k*(x1 .- X2))
    return j0.(sqrt.(reshape(sum((Δx .- im*a.β).^2, dims=1), :)))/i0(norm(a.β))
end
