...
function (a::UniformKernel)(x::AbstractVector{<:Number})
    η = sqrt(sum(x.^2))
    return j0(a.k*η)
end

function (a::UniformKernel)(x::AbstractMatrix{<:Number})::AbstractVector
    η = sqrt.(reshape(sum(x.^2, dims=1), :))
    return j0.(a.k*η)
end

function (a::UniformKernel)(x::AbstractArray{<:Number, 3})::AbstractMatrix
    _, b1, b2 = size(x)
...
function (a::UniformKernel)(x1::AbstractVector{<:Number}, x2::AbstractVector{<:Number})
    return j0(a.k*norm(x1-x2))
end

### Kernel evaluation for a batch of inputs and one output
function (a::UniformKernel)(X1::AbstractMatrix{<:Number}, x2::AbstractVector{<:Number})::AbstractVector
    Δx = sqrt.(reshape(sum(abs2, X1 .- x2, dims=1), :))
    return j0.(a.k*Δx)
end

### Kernel evaluation for an input and a batch of inputs
function (a::UniformKernel)(x1::AbstractVector{<:Number}, X2::AbstractMatrix{<:Number})::AbstractVector
    Δx = sqrt.(reshape(sum(abs2, x1 .- X2, dims=1), :))
    return j0.(a.k*Δx)
end
